				...
				if(args.length>=1){
					...
					if(args[0].equals("PI")){
						...
						if(config.CheckWorld(WUUID,WID)){
							P.sendMessage("§f[§5"+MCW.getName()+"§5]");
							...
							P.sendMessage("§f[§4"+config.L("Items")+" "+config.L("Whitelist")+"]§f  ("+Itemslist_+")");
							P.sendMessage("§f[§4"+config.L("Block")+" "+config.L("Whitelist")+"]§f  ("+Blockslist_+")");
							return true;
						}else{
